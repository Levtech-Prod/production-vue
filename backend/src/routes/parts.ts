...
/**
 * The part's name, or the refusal its absence means: `null` from the service
 * is a category that does not exist, `''` is 'custom' mode with nothing typed
 * (a generated name always carries at least the category name). Create and
 * update both need exactly this, so neither spells the two guards itself.
 */
async function requirePartName(
  client: PoolClient,
  categoryId: number,
  prefix: string,
  parameters: { parameterId: number; value: string }[],
  ...
  const name = await resolvePartNameForCategory(
    ...
    categoryId,
    prefix,
    valuesByParameterId(parameters),
  );
  if (name === null) throw new ApiError(404, ErrorCodes.CATEGORY_NOT_FOUND);
  if (!name) throw new ApiError(422, ErrorCodes.PART_NAME_REQUIRED);
  return name;
}
...
router.post('/', requireAuth, requireAdmin, async (req, res) => {
  ...
  try {
    const created = await withTransaction(async (client) => {
      const name = await requirePartName(client, data.categoryId, data.name, data.parameters);
      ...
      await writeAudit(client, 'part', part.id, 'created', {
        snapshot: {
          name,
          code: data.code,
    ...
    if (isUniqueViolation(err)) throw new ApiError(409, ErrorCodes.PART_CODE_ALREADY_EXISTS);
    throw err;
  }
});
...
router.put('/:id', requireAuth, requireAdmin, async (req, res) => {
  ...
  try {
    const updated = await withTransaction(async (client) => {
      ...
      if (partResult.rowCount === 0) throw new ApiError(404, ErrorCodes.PART_NOT_FOUND);
...
router.delete('/:id', requireAuth, requireAdmin, async (req, res) => {
  ...
  await withTransaction(async (client) => {
    ...
    if (deleteResult.rowCount === 0) throw new ApiError(404, ErrorCodes.PART_NOT_FOUND);

    // Audit rows have no FK to parts, so they survive this hard delete.
    const deleted = deleteResult.rows[0];
    await writeAudit(client, 'part', partId, 'deleted', {
